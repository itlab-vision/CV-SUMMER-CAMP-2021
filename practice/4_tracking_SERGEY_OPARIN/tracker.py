...
from common_modules.feature_distance import calc_features_similarity
from common_modules.common_objects import DetectedObject, validate_detected_object, Bbox
from common_modules.common_objects import get_bbox_center, get_dist, calc_bbox_area
from common_modules.find_best_assignment import solve_assignment_problem
from common_modules.annotation import AnnotationObject, AnnotationStorage
...
class Tracker:
    ...
    def _calc_affinity_appearance(self, track, obj):
        ...
        for i in range(len(obj.appearance_feature)):     
            mod_difference = sum([(track.last().appearance_feature[i] - obj.appearance_feature[i])**2])
    ...
    def _calc_affinity_position(self, track, obj):
        ...
        last_min_x, last_min_y, last_max_x, last_max_y = track.last().bbox
        w1 = last_max_x - last_min_x
        h1 = last_max_y - last_min_y

        obj_min_x, obj_min_y, obj_max_x, obj_max_y = obj.bbox
        w2 = obj_max_x - obj_min_x
        h2 = obj_max_y - obj_min_y

        center_last_x = last_min_x+w1/2.0
        center_last_y = last_min_y+h1/2.0
        center_obj_x = obj_min_x+w2/2.0
        center_obj_y = obj_min_y+h2/2.0
