...
def make_cat_passport_image(input_image_path, haar_model_path):
    ...
    rects = detector.detectMultiScale(resized, scaleFactor=1.1, minNeighbors=5, minSize=(75, 75))
...
def create_cat_passport(crop_image):
    ...
    passport[y1:y2, x1:x2] = cv2.resize(
        crop_image, (x2 - x1, y2 - y1), interpolation=cv2.INTER_CUBIC
    )
    # show("passport", passport)
...
def build_argparser():
    ...
    args.add_argument(
        "-i", "--input", type=str, required=True, help="Required. Path to input image"
    )
    return parser
