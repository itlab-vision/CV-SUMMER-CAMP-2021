...
def parse_cmd_output(text):
    ...
    output_answers = ""
    split_text = text.split(b"[ INFO ] ")
    
    for answers in split_text:
        if (answers.startswith((b'---answer', b'Question'))):
            output_answers += answers.decode()
...
def main():
    ...
    path_to_demo = "C:/Program Files (x86)/IntelSWTools/openvino_2021.3.394/deployment_tools/open_model_zoo/demos/bert_question_answering_demo/python/bert_question_answering_demo.py"
