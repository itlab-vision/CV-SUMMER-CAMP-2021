...
class Tracker:
    ...
    def _calc_affinity_appearance(self, track, obj):
        myobj = track.last()
        return calc_features_similarity(myobj[2], obj[2])

    def _calc_affinity_position(self, track, obj):
        myobj = track.last()
        D = get_dist(get_bbox_center(myobj[1]), get_bbox_center(obj[1]))
        w1h1 = calc_bbox_area(myobj[1])
        C = 0.66
    ...
    def _calc_affinity_shape(self, track, obj):
        myobj = track.last()
        D = get_dist(get_bbox_center(myobj[1]), get_bbox_center(obj[1]))
        w1, h1 = get_bbox_size(myobj[1])
        w2, h2 = get_bbox_size(myobj[1])
        C = 0.33
