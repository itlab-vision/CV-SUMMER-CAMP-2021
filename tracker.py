...
class Tracker:
    ...
    def _calc_affinity_appearance(self, track, obj):
        ...
        newobject = track.last()
        return calc_features_similarity(newobject[2], obj[2])
    ...
    def _calc_affinity_position(self, track, obj):
        ...
        newobject = track.last()
        D = get_dist(get_bbox_center(newobject[1]), get_bbox_center(obj[1]))
        w1h1 = calc_bbox_area(newobject[1])
        C = 0.66
        return math.exp(-C * (pow(D, 2) / w1h1))
    ...
    def _calc_affinity_shape(self, track, obj):
        ...
        newobject = track.last()
        D = get_dist(get_bbox_center(newobject[1]), get_bbox_center(obj[1]))
        w1, h1 = get_bbox_size(newobject[1])
        w2, h2 = get_bbox_size(newobject[1])
        C = 0.33
        return math.exp(-C * ((w1-w2)/w1 + (h1-h2)/h1))
