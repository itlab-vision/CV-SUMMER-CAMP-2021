...
class Tracker:
    ...
    def _calc_affinity_appearance(self, track, obj):
        last_object = track.last()
        return calc_features_similarity(last_object[2], obj[2])

    def _calc_affinity_position(self, track, obj):
        last_object = track.last()
        dist = get_dist(get_bbox_center(last_object[1]), get_bbox_center(obj[1]))
        wh = calc_bbox_area(last_object[1])
        const = 0.66
        return math.exp(-const * (dist**2) / wh)

    def _calc_affinity_shape(self, track, obj):
        last_object = track.last()
        dist = get_dist(get_bbox_center(last_object[1]), get_bbox_center(obj[1]))
        w1, h1 = get_bbox_size(last_object[1])
        w2, h2 = get_bbox_size(obj[1])
        const = 0.33
        return math.exp(-const * (w1-w2) / w1 + (h1-h2)/h1)
